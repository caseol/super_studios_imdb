...
class IMDb:
    ...
    def getPeople(self, imdb_id, people_type, details=None, InList=False, name_id=False):
        try:
            ...
            for person in people_details:
                if "name" in person:
                    if name_id:
                        person_to_add = {people_type: person["name"], "id": person["url"][6:-1]}
    ...
    def getDistributorsInfo(self, imdb_id):
        info = []
        try:
            #imdb_id = IMDb.getIdFromSearch(self, imdb_id)
            source = IMDb.getPage(IMDb.url+"/title/"+imdb_id+"/companycredits/")

            div = source.find("div", {"data-testid": "sub-section-distribution"})

            for li in div.find_all("li", {"class": "ipc-metadata-list__item ipc-metadata-list-item--link"}):
                a = li.find("a", {"class": 'ipc-metadata-list-item__label--link'})
                info.append(a.text)
        except:
            return []
